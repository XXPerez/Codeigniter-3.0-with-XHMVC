...
//			$output .= $this->compile_queries();
    ...
    function compile_queries()
    {
		...
		{
			$count++;
			$hide_queries = false;//(count($db->queries) > $this->_query_toggle_count) ? ' display:none' : '';
    ...
    protected function compile_unittests()
    {
        $output = '';
        if (isset($this->CI->unittest))
        {
		$output  = "\n\n";
		$output .= '<fieldset id="ci_profiler_memory_usage" style="border:1px solid #5a0099;padding:6px 10px 10px 10px;margin:20px 0 20px 0;background-color:#111">';
		$output .= "\n";
		$output .= '<legend style="color:#5a0099;">&nbsp;&nbsp;'.$this->CI->lang->line('profiler_unittests').'&nbsp;&nbsp;</legend>';
		$output .= "\n";
                $output .= $this->CI->unittest->report();
		$output .= "</fieldset>";
        }
